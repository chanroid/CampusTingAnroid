...
public class SHA256 {
	 ...
	 */
	private static void SHA256_Init(SHA256_INFO Info) {
	 ...
	 *            : 암호문
	 */
	private static void SHA256_Close(SHA256_INFO Info, byte[] pszDigest) {
		int i, Index;
	...
	private static class SHA256_INFO {
		private int uChainVar[] = new int[SHA256_DIGEST_VALUELEN / 4];
		private int uHighLength;
		private int uLowLength;
		private byte szBuffer[] = new byte[SHA256_DIGEST_BLOCKLEN];
	}
	...
	private static class Common {
		...
		private static void arraycopy(byte[] dst, byte[] src, int length) {
		...
		private static void arrayinit(byte[] dst, byte value, int length) {
			for (int i = 0; i < length; i++) {
				dst[i] = value;
			}
		}
		...
		private static void memcpy(int[] dst, byte[] src, int length, int ENDIAN) {
			int iLen = length / 4;
		...
		private static byte get_byte_for_int(int[] src, int b_offset, int ENDIAN) {
			if (ENDIAN == BIG_ENDIAN) {
				int shift_value = (3 - b_offset % 4) * 8;
				int mask_value = 0x0ff << shift_value;
		...
		private static int URShift(int x, int n) {
			if (n == 0)
				return x;
			if (n >= 32)
				return 0;
		...
		private static final long INT_RANGE_MAX = (long) Math.pow(2, 32);

		private static long intToUnsigned(int x) {
			if (x >= 0)
				return x;
